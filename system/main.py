...
# 创新算法 - FedLGO (Landscape-Guided Optimization with Gradient Correction)
# 含消融变体: woLGO(无损失地形引导), woMGC(无动量梯度校正), woBoth(基线)
from flcore.servers.serverlgo import FedLGO, FedLGO_woLGO, FedLGO_woMGC, FedLGO_woBoth
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        # 创新算法 - FedLGO及消融变体
        elif base_algorithm == "FedLGO":
            server = FedLGO(args, i)
        
        elif base_algorithm == "FedLGO_woLGO":
            server = FedLGO_woLGO(args, i)
        
        elif base_algorithm == "FedLGO_woMGC":
            server = FedLGO_woMGC(args, i)
        
        elif base_algorithm == "FedLGO_woBoth":
            server = FedLGO_woBoth(args, i)

        else:
            raise NotImplementedError(
                f"算法 '{base_algorithm}' 未实现。\n"
                f"支持的算法:\n"
                f"  传统FL: FedAvg, FedProx, SCAFFOLD, MOON, FedGen\n"
                f"  个性化FL: FedProto, FedRep, Ditto, PerAvg\n"
                f"  创新算法: FedLGO, FedLGO_woLGO, FedLGO_woMGC, FedLGO_woBoth"
            )
...
if __name__ == "__main__":
    ...
    # ==================== FedLGO 参数 ====================
    # FedLGO 景观引导参数 (LGO)
    parser.add_argument('--lgo_tau_max', type=float, default=2.0,
                        help='FedLGO: Initial temperature for client selection (default: 2.0)')
    parser.add_argument('--lgo_tau_min', type=float, default=0.5,
                        help='FedLGO: Minimum temperature (default: 0.5)')
    parser.add_argument('--lgo_alpha_lr', type=float, default=0.3,
                        help='FedLGO: Adaptive learning rate scaling factor (default: 0.3)')
    parser.add_argument('--lgo_delta_weight', type=float, default=0.2,
                        help='FedLGO: Landscape score weight in aggregation (default: 0.2)')
    parser.add_argument('--lgo_hutchinson_samples', type=int, default=10,
                        help='FedLGO: Number of Hutchinson samples for Hessian trace estimation (default: 10)')
    
    # FedLGO 梯度校正参数 (MGC)
    parser.add_argument('--lgo_beta', type=float, default=0.9,
                        help='FedLGO: Momentum coefficient (default: 0.9)')
    parser.add_argument('--lgo_gamma', type=float, default=0.5,
                        help='FedLGO: Gradient correction strength (default: 0.5)')
    parser.add_argument('--lgo_lambda', type=float, default=0.1,
                        help='FedLGO: Momentum contribution to update (default: 0.1)')
    
    # FedLGO 消融实验开关
    parser.add_argument('--lgo_use_landscape', type=lambda x: str(x).lower() == 'true', default=True,
                        help='FedLGO Ablation: Enable landscape-guided optimization (default: True)')
    parser.add_argument('--lgo_use_correction', type=lambda x: str(x).lower() == 'true', default=True,
                        help='FedLGO Ablation: Enable gradient correction (default: True)')
